...
public class productDAO extends DBContext {
    ...
    public void updateProduct(Product product) {
        ...
        String sql = "update product set category_id=? ,product_name=? ,product_price=? ,product_describe=? ,quantity=? where product_id=?";
        try {
            ps = connection.prepareStatement(sql);
...
//            ps.setString(6, product.getImg());
    ...
    public List<Product> getTop12Product() {
        List<Product> list = new ArrayList<>();
        String sql = "SELECT TOP 12 p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img FROM product p ORDER BY NEWID()";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public int CountProductLow() {
        int count = 0;
        String sql = "SELECT COUNT(*) as 'count'\n"
                + "  FROM product where quantity < 30 ";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public List<Product> getCollection() {
        ...
        String sql = " SELECT p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img FROM product p\n"
                + "  where product_name like '%Champion%'";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public List<Product> getCollectionProductToHigh() {
        List<Product> list = new ArrayList<>();
        String sql = " SELECT p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img FROM product p\n"
                + "  where product_name like '%Champion%'\n"
    ...
    public List<Product> getCollectionProductToLow() {
        List<Product> list = new ArrayList<>();
        String sql = "  SELECT p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img FROM product p\n"
                + "  where product_name like '%Champion%'\n"
    ...
    public List<Product> getCollectionProductAZ() {
        List<Product> list = new ArrayList<>();
        String sql = "  SELECT p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img FROM product p\n"
                + "  where product_name like '%Champion%'\n"
